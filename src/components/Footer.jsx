  ...
  const socialLinks = [
    {
      id: "github",
      icon: <FaGithub />,
      url: "https://github.com/KhalilovDev0",
  ...
  return (
    <footer className="w-full bg-white/10 backdrop-blur-md border-t border-white/5 text-white shadow-xl relative z-10">
      <div className="max-w-8xl mx-auto flex flex-col sm:flex-row justify-between items-center gap-4 py-8 px-4 sm:px-6 lg:px-8">
          ...
          {socialLinks.map((link) => (
            <li key={link.id}>
              <a
                ...
                className="block text-slate-400 hover:text-cyan-400 transition-all duration-300 transform hover:scale-110 lg:text-2xl"
              >
                {link.icon}
              </a>
            </li>
        ...
        <div className="text-xs md:text-sm text-slate-400 text-center sm:text-right font-medium tracking-wide">
          © {currentYear}{" "}
          <span className="text-white hover:text-cyan-400 transition-colors duration-300 cursor-pointer font-semibold">
            Khalilov Dev
          </span>
          . All rights reserved.
        </div>
